...
缓存可以用来提高数据的查询速度，但缓存容量有限，当缓存达到容量上限时，需要删除部分数据来添加新的数据，根据删除数据的策略不同可以分为FIFO、LRU和LFU三种。FIFO先进先出，可以通过有序Map来实现。
<!-- more -->

### LRU
LRU（Least Recently Used）最近最少使用算法，LRU的逻辑是**如果数据最近被访问过，那么未来也极有可能被访问**，当缓存达到容量上限删除最久没有被访问过的数据。
...
``` graphviz
digraph {
    graph [label="LRU缓存", splines=false, fontsize=10]
	node  [fontsize=10]
...
### LFU
LFU（Least Frequently Used）最不经常使用，LFU的逻辑是**如果数据最近一段时间访问次数很少，那么在未来被访问的可能性也很小**，当缓存达到容量上限删除访问次数最少的元素，如果有多个元素访问次数最少，按照访问时间顺序删除最久未被访问的元素。
...
``` graphviz
digraph {
	graph [label="LFU缓存", splines=false, fontsize=10, ranksep=0.4]
    node  [fontsize=10]
